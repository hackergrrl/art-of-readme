...
Некоторые опытные разработчики на Nodejs объединились, чтобы создать [рекомендованные  списки] (https://github.com/sindresorhus/awesome-nodejs) качественных модулей.
Разработчики использовали долгие годы изучения сотен модулей для того чтобы поделиться с новичками самыми сливками: лучшими модулями в каждой категории. Также новые удобные модули, разработанные доверенными людьми, могут рассылатья по RSS или в почтовых рассылках.

Как насчет социального графа? Эта идея стимулировала создание
[node-modules.com] (http://node-modules.com/), замену стандартного поиска npm, использующую  ваш социальный граф Github для поиска модулей, которые нравятся или были сделаны вашими друзьями.

Конечно же есть и [поиск] (https://npmjs.org), встроенный в npm: безопасный и являющийся точкой входа для всех новых разработчиков.

Независимо от того какой подход вам больше нравится, и не смотря на то где ищущий модули в конечном счете находит себя: на [npmjs.org] (https://npmjs.org) или [Github.com] (https://github.com) или где-то еще, у всех у них перед лицом окажется ваше README. Так как пользователи будут неизбежно заканчивать свой осмотр именно здесь, что можно сделать, чтобы краткое впечатление от модуля было максимально эффективным?
